...
export function BotonDelPortal({
    ...
    variante?: 'lleno' | 'claro' | 'contorno' | 'sobre-color';
    className?: string;
}) {
    ...
    return (
        <button
            ...
            className={cn(
                ...
                variante === 'claro' && 'bg-white text-indigo-700 shadow-sm hover:bg-indigo-50 focus-visible:ring-white/50',
                variante === 'contorno' &&
                    'border border-slate-200 bg-white text-slate-800 shadow-sm hover:bg-slate-50 focus-visible:ring-indigo-200',
                // Segunda llamada sobre el fondo índigo: la primera ya es la blanca.
                variante === 'sobre-color' && 'border border-white/60 text-white hover:bg-white/10 focus-visible:ring-white/50',
                className
            )}
        >
            <span>{children}</span>
